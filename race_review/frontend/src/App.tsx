...
import { feet, interpolateTelemetry, mph, unpackTelemetry, type TelemetrySample } from "./telemetry";
...
function Review({ sessionId, onBack }: { sessionId: string; onBack: () => void }) {
  ...
  const chartData = useMemo(() => ({
    ...
    series: [
      { label: "Speed mph", values: samples.map((sample) => mph(Number(sample.speed_mps))), color: "#f2f5f6" },
      { label: "Lateral g", values: samples.map((sample) => Number(sample.lateral_g)), color: "#ff5038" },
      { label: "Longitudinal g", values: samples.map((sample) => Number(sample.longitudinal_g)), color: "#ffb238" },
    ],
  }), [samples]);
  ...
  return <main className="review-shell">
    <header className="topbar"><button className="wordmark wordmark--button" onClick={onBack}><span>GR86</span> / REVIEW</button><div><strong>{session.name}</strong><small>{session.chapters.length} CHAPTER{session.chapters.length === 1 ? "" : "S"}</small></div><EditPanel session={session} corners={corners} onSaved={load} /></header>
    ...
    <div className="review-grid">
      ...
      <aside className="track-panel"><div className="section-heading"><h2>Track</h2><span>OFFLINE ROUTE</span></div><TrackMap samples={samples} current={current} corners={corners} track={session.edits.track} />
        <div className="current-corner"><small>NEXT · {feet(Number(current?.lap_distance_m ?? 0)).toFixed(0)} FT</small><strong>{corners.find((corner) => corner.apex_distance_m > Number(current?.lap_distance_m ?? 0))?.name ?? corners[0]?.name ?? "—"}</strong></div>
      </aside>
    ...
    <footer className="player-bar"><button className="transport" onClick={togglePlayback}>▶ / Ⅱ</button><Scrubber time={time} duration={session.duration_seconds} chapters={session.chapters} laps={laps} corners={corners} onSeek={seek} />
      <select aria-label="Playback speed" defaultValue="1" onChange={(event) => { if (video) video.playbackRate = Number(event.target.value); }}><option value="0.25">0.25×</option><option value="0.5">0.5×</option><option value="1">1×</option><option value="1.5">1.5×</option><option value="2">2×</option></select></footer>
  </main>;
}
